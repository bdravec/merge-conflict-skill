...
**Pane A — sanity check, then vLLM serve:**

The moment Pane A's prompt flips from `submit01` → `gnodeXX`, sanity-check the allocation before anything else (the `--time` clock is already running):
