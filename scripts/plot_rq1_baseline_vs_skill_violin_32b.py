"""
plot_rq1_baseline_vs_skill_violin_32b.py — RQ1 baseline-vs-skill split violins, Qwen3-32B (#82)

Large-pair sibling of plot_rq1_baseline_vs_skill_violin.py (#68). The 8B script
stacks two model subplots (Qwen3-8B / Apertus-8B); the large pair only has
Qwen3-32B so far (Apertus-70B sys runs pending), so each figure here is a single
per-bucket split violin: LEFT half = no-skill baseline, RIGHT half = skill-vX-sys.
Metric: max(edit, winnowing). Rendering/tiering identical to the 8B script.

Data is sys-only per the large-pair convention: baseline from the shared 32B
no-skill file, skill from each version's sys-only run.

Outputs:
  - docs/figures/rq1_baseline_vs_v1_sys_max_32b.png
  - docs/figures/rq1_baseline_vs_v2_sys_max_32b.png
  - docs/figures/rq1_baseline_vs_v2.1_sys_max_32b.png

Run from repo root:
    python scripts/plot_rq1_baseline_vs_skill_violin_32b.py
"""
...
def plot_rq1(version: str):
    ...
    fig.suptitle(
        f"RQ1: does skill-v{version}-sys help vs no-skill?  {MODEL_NAME}, "
    ...
    plt.tight_layout()
    out_path = os.path.join(FIG_DIR, f"rq1_baseline_vs_v{version}_sys_max_32b.png")
